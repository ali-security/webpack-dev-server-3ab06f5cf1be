...
class Server {
  ...
  async initialize() {
    ...
    this.setupCrossOriginHeaderCheck();
    ...
    (this.webSocketProxies).forEach((webSocketProxy) => {
      ...
      (this.server).on("upgrade", (req, socket, head) => {
        if (hmrPath && typeof req.url === "string") {
          ...
          // https://github.com/websockets/ws/blob/8.13.0/lib/websocket-server.js#L214
  ...
  setupCrossOriginHeaderCheck() {
    ...
    (this.app).all(
      ...
      (req, res, next) => {
        ...
        if (this.checkHeader(headers, headerName, true, false)) {
          next();
          return;
        }
  ...
  createWebSocketServer() {
    ...
    (this.webSocketServer).implementation.on(
      ...
      (client, request) => {
        ...
        // an explicit IP address in the "origin" header is only trusted when it
        // is the same origin as the "host" header, otherwise an untrusted
        // remote IP host can send requests
        if (
          !headers ||
          !this.checkHeader(headers, "host", true) ||
          !this.checkHeader(headers, "origin", this.isSameOrigin(headers))
        ) {
   ...
   * @returns {boolean}
   */
  checkHeader(headers, headerToCheck, allowIP, validateHost = true) {
    ...
    try {
      ...
      // a malformed header value (for example an invalid IPv6 literal) makes
      // the parser throw, treat it as a disallowed host instead of crashing
      return false;
    }
    ...
    // These implicitly allowed hostnames are only trusted when `validateHost`
    // is true, otherwise only hostnames explicitly allowed by the user through
    // the `allowedHosts` option are accepted.
  ...
  isUserCORSWildcardEnabled() {
    ...
    return Object.entries(headers).some(
      ([key, value]) =>
        key.toLowerCase() === "access-control-allow-origin" && isWildcard(value)
    );
  }
  ...
  isSameOrigin(headers) {
    ...
    // use the node url-parser to retrieve the hostname from both headers,
    // the port is not part of the comparison, like for `allowedHosts`
    /** @type {string | null} */
    let origin = null;
    /** @type {string | null} */
    let host = null;
    ...
    try {
      origin = url.parse(
        // if originHeader doesn't have scheme, add // for parsing.
        /^(.+:)?\/\//.test(originHeader) ? originHeader : `//${originHeader}`,
        false,
        true
      ).hostname;
      host = url.parse(
      ...
      // a malformed header value (for example an invalid IPv6 literal) makes
      // the parser throw, a request we cannot parse is never same-origin
      return false;
    }
